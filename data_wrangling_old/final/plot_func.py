...
def couplot(frame,col,title,xlab,ylab,nums=True,leg='best',ymax=None,save=False):    
    ...
    edgecolor='w',transparent=True,bbox_inches=None, pad_inches=0.1, frameon=False)
...
def barplot(frame,col,title,xlab,ylab,nums=True,save=False):    
    fig,ax = plt.subplots(figsize=(10,5))
    ax = sns.countplot(x=col,data=frame,alpha=0.75, order=frame['{}'.format(col)].value_counts().index)
    max_value = frame['{}'.format(col)].value_counts().max()
